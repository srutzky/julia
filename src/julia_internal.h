...
// `jl_bt_element_t` values, in two forms which are understood by the GC:
//
...
//
// An extended entry is made up of several jl_bt_element_t values:
//
//   JL_BT_NON_PTR_ENTRY  - Special marker to distinguish extended entries
//   tags                 - A bit packed uintptr_t containing the number of
//                          following GC-managed values and entry size
//   [ngc  * jl_value_t]  - GC managed data
//   [nptr * uintptr_t]   - non-GC-managed data
//
...
STATIC_INLINE uintptr_t jl_bt_pack_extended_entry(int ngc, int nptr,
